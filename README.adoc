...
chl := changelogger.FindLogger(res[0].MergeLoggers) // <4>
...
The above sample shows how to report and desired a certain value including how to access a merge logger. It relies on a manager to be configured and built. It is possible to have several managers with different configuration as well as chain managers, much like a, middleware chain.
