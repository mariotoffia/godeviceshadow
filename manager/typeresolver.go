...
// to resolve. If that fails, it will use `persistencemodel.ModelIndependentPersistenceID.Name` and the `model.TypeRegistry` to get at
// a last resort.
//
// NOTE: It is possible to provide with a set of _id_(s).
func (mgr *Manager) ResolveType(name string, id ...persistencemodel.ModelIndependentPersistenceID) (model.TypeEntry, bool) {
